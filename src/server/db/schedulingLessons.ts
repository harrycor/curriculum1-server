...
const getAllCoachesAndAdminsByTenant = async (tenant: string) => {
  return await Query(
    ...
    select 
      ...
      where u.tenant = ? and (u.role = "admin" or u.role= "coach") and u.is_active=1;
  `,
    [tenant]
  );
};
...
FROM
    ...
    join users u on plb.wrestler_user_id = u.id
WHERE
    plb.coaches_user_id = ?
...
const getCoachesFullPrivateLessonsScheduleByDay = async (
  coachesId: number | string,
  date: string
) => {
  return await Query(
...
WHERE
    ...
    ORDER BY plb.date_of_lesson, plb.start_time;
  `,
    [coachesId, coachesId, date]
  );
};

const getPhoneNumberByUserId = async (userId: string) => {
  return await Query(
    `select phone_number from personal_info where user_id = ?;`,
    [userId]
  );
};
...
const postNewPrivateLesson = async (
  ...
  return await Query(
    ...
    date_of_lesson, start_time, duration, notes, series_name, lesson_scheduled_by_user_id)
    values(?, ?, ?, ?, ?, ?, ?, ?);`,
    [
      coachId,
      wrestlerId,
...
export default {
  ...
  getCoachesFullPrivateLessonsScheduleByDay,
